var vampire = new Char();
var vampireWheelDamage = 1;
function makeVampire() {
    vampire.passive = function () {
    };
    vampire.cooldownINIT = calculateSkillHaste();
    vampire.skillQ = function () {
        if (playerDistance > players[team].spec.range * 1.5)
            return;
        vampire.cooldown.q = vampire.cooldownINIT.q;
        var isCritical = Math.random() < players[team].spec.criticP / 100 && hasItem("3_shadowflame");
        var damage = skillInfo.q.damage + players[team].spec.ap * skillInfo.q.ap;
