setInterval(() => {
    ...
    if ((keyDown.d) && (keyDown.w) ) {
        players.ally.selector.style.left = `${ position.x + (players.ally.moveSpd / Math.SQRT2) }px`;
        players.ally.selector.style.top = `${ position.y - (players.ally.moveSpd / Math.SQRT2) }px`;
        ...
        players.ally.selector.style.top = `${ position.y - players.ally.moveSpd }px`;
        ...
        players.ally.selector.style.left = `${ position.x - players.ally.moveSpd }px`;
    ...
    if (keyDown.arrowright) cameraPosition.x += cameraSpd;
    if (keyDown.arrowleft) cameraPosition.x -= cameraSpd;
    if (keyDown.arrowup) cameraPosition.y -= cameraSpd;
    if (keyDown.arrowdown) cameraPosition.y += cameraSpd;
    ...
    if (cameraPosition.x > 2480) cameraPosition.x = 2480;
